...
.. csv-table:: New Module Versions
    ...
    ``genie``, v26.6 
    ``genie.lamp``, v26.6 
    ``genie.libs.health``, v26.6 
    ``genie.libs.clean``, v26.6 
    ``genie.libs.conf``, v26.6 
    ``genie.libs.filetransferutils``, v26.6 
    ``genie.libs.ops``, v26.6 
    ``genie.libs.parser``, v26.6 
    ``genie.libs.robot``, v26.6 
    ``genie.libs.sdk``, v26.6 
    ``genie.telemetry``, v26.6 
    ``genie.trafficgen``, v26.6 
...
* genie.conf.base.api
    * Modified CleanAPI.render_clean_template
        * Preserve clean stage configuration even when the stage is not listed in the template's default order
...
* iosxr
    * update the sned_to_cli to just return the output, since the parent check for the failure

* filetransferutils
    * Modified HTTPS protocol support
        * Added a concrete HTTPS FileUtils protocol module so remote filesize
...
* iosxe
    * Modified Platform
        * Added a normalized, OS-agnostic `members` collection to the
...
* base
    * Modified Platform schema
        * Added `members` as a top-level Any() entry so OS-specific Platform
