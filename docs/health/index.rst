...
The health check is driven by a `health_file` which is provided at run time. The `health_file` can be a file path or a URL. There is two different mechanism to run the checks:
